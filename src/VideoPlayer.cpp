...
#define PL_FFMPEF_IMPLEMENTATION
...
namespace videoplayer {
    struct VideoPlayer::Impl {
        ...
        static void VideoFinished(plm_t* plm, void* user);
        static void videoCallback(plm_t* plm, plm_frame_t* frame, void* user);
        static void audioCallback(plm_t* plm, plm_samples_t* samples, void* user);
    ...
    void VideoPlayer::Impl::VideoFinished(plm_t* plm, void* user) {
        if (!user) return;
        VideoPlayer* videoPlayer = static_cast<VideoPlayer*>(user);
        if (videoPlayer->m_onVideoEnd) videoPlayer->m_onVideoEnd();
    };
    ...
    bool VideoPlayer::init(std::filesystem::path const& path, bool loop) {
        ...
        #ifdef PL_PLM_SET_ENDING_CALLBACK
            plm_set_ending_callback(stream, Impl::VideoFinished, this);
        #endif
    ...
    void VideoPlayer::Impl::videoCallback(plm_t* plm, plm_frame_t* frame, void* user) {
    ...
    void VideoPlayer::Impl::audioCallback(plm_t* plm, plm_samples_t* samples, void* user) {
        VideoPlayer* self = (VideoPlayer*) user;
